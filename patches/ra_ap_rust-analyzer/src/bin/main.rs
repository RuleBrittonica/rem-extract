...
fn setup_logging(log_file_flag: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    ra_ap_rust_analyzer::tracing::Config {
        writer,
...
fn run_server() -> anyhow::Result<()> {
    tracing::info!("server version {} will start", ra_ap_rust_analyzer::version());
    ...
    let server_capabilities = ra_ap_rust_analyzer::server_capabilities(&config);

    let initialize_result = lsp_types::InitializeResult {
        capabilities: server_capabilities,
        server_info: Some(lsp_types::ServerInfo {
            name: String::from("rust-analyzer"),
            version: Some(ra_ap_rust_analyzer::version().to_string()),
        }),
    ...
    match (ra_ap_rust_analyzer::main_loop(config, connection), io_threads.join()) {
